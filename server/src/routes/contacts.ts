...
router.get('/', [checkAuth], async (req: Request, res: Response) => {
	try {
		const contacts = await pool.query(`SELECT id, first_name, last_name, contact_type, address, created_at FROM contacts WHERE origin_id = $1 order by first_name, id limit $2 offset $3`, [req.session.userId, req.query.limit, req.query.offset]);
...
router.post(
	'/',
	[checkAuth, check('contactType', 'contactType is required').not().isEmpty(), check('firstName', 'firstName is required').not().isEmpty(), check('lastName', 'lastName is required').not().isEmpty()],
