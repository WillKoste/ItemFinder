...
import {CartProduct, Product} from '../types/redux';
...
export const addItemToCart = (item: Product, cartItems: CartProduct[], qty: number) => async (dispatch: any) => {
	...
	const parsedCart: CartProduct[] = JSON.parse(AsyncStorage.getItem('cart') as any);
	...
	if (indexIWant === -1) {
		...
		console.log({newArr});
		AsyncStorage.setItem('cart', JSON.stringify(newArr));
		let totalPrice = 0;
		newArr.forEach((z) => {
			totalPrice += z.cart_subtotal;
		});
		console.log({totalPrice});
		AsyncStorage.setItem('cartTotal', JSON.stringify(totalPrice));
		dispatch({
			type: ADD_CART,
			payload: newArr,
			total: totalPrice
		...
		AsyncStorage.setItem('cart', JSON.stringify(parsedCart));
		let total = 0;
...
export const updateCartQty = (itemId: number, newQty: number) => async (dispatch: any) => {
	const parsedCart: CartProduct[] = JSON.parse((await AsyncStorage.getItem('cart')) as any);
...
export const removeItemFromCart = (productId: number) => async (dispatch: any) => {
	const cartItems: CartProduct[] = JSON.parse((await AsyncStorage.getItem('cart')) as string);
	...
	AsyncStorage.setItem('cartTotal', JSON.stringify(priceTotal));
...
export const setCartItems = (cartItemz: Product[]) => async (dispatch: any) => {
	AsyncStorage.setItem('cart', JSON.stringify(cartItemz));
	const cartTotal = AsyncStorage.getItem('cartTotal');
	console.log({YAYAYA: cartTotal});
	dispatch({
		type: SET_CART,
		payload: cartItemz,
		total: +JSON.parse((cartTotal as any) ? (cartTotal as any) : '0')
	});
};
