...
import {Favorite} from '../types/general';

export const getFavorites = (userId: number, productId?: number) => async (dispatch: any) => {
	try {
		const productIdQuery = `&productId=${productId}`;
		const res: AxiosResponse<{favorites: Favorite[]}> = await customAxios.get(`/api/v1/favorites?userID=${userId}${productId ? productIdQuery : ''}`);
		dispatch({
			type: GET_FAVORITES,
